...
function Page(props) {
    ...
    const API_URL = `${MAKEUP_API_BASE_URL}/v1/products.json?brand=maybelline`; // 프록시된 API URL (env)

    // 데이타 가져오기
    const getDate = async () => {
        try {
            setLoading(true); // 로딩 상태 시작
            const response = await axios.get(API_URL); // axios를 사용한 API 호출 (async, await같이)
